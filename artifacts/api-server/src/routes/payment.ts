...
router.post("/create-order", async (req: Request, res: Response) => {
  try {
    ...
    const amountInSmallestUnit = currency === "USD" ? Math.round(Number(amount) * 100) : Math.round(Number(amount) * 100);
    const order = await razorpay.orders.create({ amount: amountInSmallestUnit, currency });
